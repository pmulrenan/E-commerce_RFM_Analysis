...
-- How many times each customer made a purchase and most frequent purchasers
...
-- RFM Analysis: Segement customers based on their recent purchase, number of purchases, and total spend

-- Step 1: Calculate Recency (to end of Q1 2024)

WITH Recency AS (
	SELECT
		CustomerID,
		DATEDIFF('2024-03-31', MAX(InvoiceDate)) AS Recency
	FROM
		customer_transactions
	GROUP BY
		CustomerID
...
-- Combine Results for RFM Analysis
...
JOIN 
	TotalSpend AS M ON R.CustomerID = M.CustomerID
ORDER BY
	CustomerID;
